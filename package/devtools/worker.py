...
class AsyncDevtoolsWorker:
        ...
        state.update(
            {
                "status": "executing",
                "target_path": path,
                "message": str(prepared.get("message") or "请在小程序内点击任意已有可点击控件完成跳转"),
    ...
    async def cleanup_pending_jump_navigation(self, jump_navigator, appid: str, path: str) -> None:
        """兼容不同 navigator 签名，尽力清理待点击的跳转悬浮层。"""
    ...
    async def start_transition(self, command: dict) -> None:
        ...
        cdp_port = 0
        try:
            cdp_port = self.cdp_port_finder(self.cdp_port_start_from_command(command))
            while True:
                ...
                try:
                    ...
                    if retry_start > CDP_PORT_END:
                        raise
                    retry_port = self.cdp_port_finder(retry_start)
                    self.apply_session_state(
                        session,
                        status="starting",
    ...
    async def recover_route_context(self, command: dict, recovery_page: dict, failures: list[dict]) -> None:
        ...
        try:
            await self.ensure_route_session(command)
            result = await self.route_navigator.visit_route(route, is_tabbar=bool(recovery_page.get("is_tabbar")))
        except Exception as exc:
