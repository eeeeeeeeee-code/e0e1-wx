...
class MonitorFileMixin:
    ...
    def prepare_root_path(self) -> None:
        """Safely clean stale top-level wx directories before monitoring starts."""
        for packages_root in self.packages_roots(create_missing=True):
            deleted_count = cleanup_wx_directories(packages_root)
            if deleted_count:
                self.emit({"type": "info", "message": f"已清理 {deleted_count} 个旧目录：{packages_root}"})
