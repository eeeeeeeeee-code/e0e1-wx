...
class MonitorScanMixin:
    async def scan_once(self) -> None:
        ...
        try:
            ...
            new_dir_names = sorted(
                set(current_dirs) - set(self.previous_dirs),
                key=lambda name: current_dirs.get(name, 0.0),
            )
            windows = await self.stable_windows(retry=bool(new_dir_names))
            new_groups = self.group_new_dirs([(name, current_dirs[name]) for name in new_dir_names])
    ...
    def bind_new_dir_groups(self, new_groups: list[tuple[list[str], float]], windows: list[dict]) -> None:
        ...
        for wxids, created_at in new_groups:
            candidates = [
                (index, window)
